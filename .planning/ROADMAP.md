...
- [x] 25-03-migration-toast-PLAN.md — IsSessionVisibilityMigrationToastVisible ObservableProperty + InitializeAsync first-launch trigger + DismissMigrationToast RelayCommand (synchronous SaveSettings) + MainView InfoBar + 2 resw key pairs (DROPDOWN-05)
**UI hint**: yes
...
**Success Criteria** (what must be TRUE):
  1. A second time label below the 5h-window countdown shows the absolute reset time using `UsageResponse.FiveHour.ResetsAt` ("Mo 1.5. 16:30" or "Wed 14:30" depending on `CultureInfo.CurrentUICulture`); the label is hidden — not "—" — when `ResetsAt` is null; the field order comes from the active language's `NextWindowLabelPattern` resw entry *(not from `l:Uids.Uid` as originally written — the label is a ViewModel string, so no XAML uid can reach it)*
  2. A new "Re-detect organization" button on the Settings Account tab calls `IClaudeApiService.ListAvailableOrganizationsAsync` (existing `/api/organizations` endpoint), shows orgs in a `ContentDialog` (name + uuid), and on selection persists the new org-id and clears the now-stale usage history; the next poll reads the new org-id, with no logout and no re-authentication *(corrected on 2026-08-06 after review finding 18 — see REQUIREMENTS.md ORGID-02: the session token is not org-scoped, only the request URL is, and the `MainViewModel.Logout` this criterion named was an unreachable duplicate that is now deleted; the one logout lives in `SettingsViewModel.Logout`)*
  ...
  5. `SettingsViewModel.LastFetchRelativeTime` reads from new resw keys `LastFetchJustNow`, `LastFetchMinutesAgo`, `LastFetchHoursAgo`, `LastFetchDaysAgo`, `LastFetchNever` in both `de-DE/Resources.resw` and `en-US/Resources.resw`; all ~30 new resw keys across NEXTWIN/ORGID/PRICING/L10N exist in both locales and are validated by the extended `ResourceCoverageTests` *(the dotted `LastFetchRelative.*` names this criterion originally specified had to be flattened: WinUI3Localizer 2.3.0 keys its dictionary on the text before the first `.`, so every one of them resolved to an empty label)*
**Plans**: 4 plans
Plans:
**Wave 1**
...
- [x] 27-02-nextwin-label-PLAN.md — NEXTWIN-01..03: FiveHourNextWindowText + IsFiveHourNextWindowVisible ObservableProperty in MainViewModel + RecomputeNextWindowLabel helper at 4 call-sites + OnIsSessionExpiredChanged partial + MainView.xaml TextBlock below countdown + 2 NextWindow format-string resw keys
