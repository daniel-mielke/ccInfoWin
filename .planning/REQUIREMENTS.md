...
- [x] **RENAME-08**: Display-layer integration: `MainViewModel.RefreshSessionList` resolves the final display name as `_sessionNameStore.GetCustomName(s.Id) ?? s.DisplayName` — `JsonlService` stays storage-free.
...
- [x] **DROPDOWN-04**: A new Settings option `SessionVisibilityWindowDays` (default 30, options 7 / 30 / 90 / 0=unlimited) appears in the General Settings tab as a ComboBox. Changing it persists to `settings.json`, and the next session-list refresh applies the new filter at the display layer in `MainViewModel.RefreshSessionList` — NOT in `JsonlService` (stats/cost aggregation must keep all data). *(Amended after review finding 37: the original wording mandated a `SessionVisibilityChangedMessage` broadcast. That channel provably never had a live recipient — `MainView` is unloaded, and its transient ViewModel unregistered, for the whole time `SettingsView` is on screen — so the message was deleted. `RefreshSessionList` re-reads `settings.json` on every call, which is what actually satisfied this requirement all along.)*
- [x] **DROPDOWN-05**: Existing installs see a one-time toast notification on first launch after upgrade ("Sessions older than 30 days are now hidden — adjustable in Settings"); tracked by a `SessionVisibilityMigrationShown` boolean in `AppSettings`.
...
- [x] **ORGID-02**: Selecting a different org persists the new org-id to the `claude-org` Credential Manager key and clears `usage-history.json` — that history belongs to the previous org and would splice two unrelated utilization curves into one chart. The next poll picks the new org up through `ICredentialService.GetOrganizationId()`; no logout and no re-authentication. *(Corrected on 2026-08-06 after review finding 18. The original wording mandated "the existing `MainViewModel.Logout` sequence … switching orgs requires re-authentication because the WebView2 cookie jar is per-org-context". Both halves were wrong: the session cookie is not scoped to an org — only the request URL carries the org id, which is why `SettingsViewModel.OpenOrgPickerAsync` deliberately does not log out — and `MainViewModel.Logout` was an unreachable duplicate bound in no XAML, now deleted. The one logout contract lives in `SettingsViewModel.Logout`, reached from the Account tab's Abmelden button.)*
...
- [~] **PRICING-01**: When pricing data is unavailable, a dedicated `IsPricingError` flag in `MainViewModel` is set to true; an `InfoBar` (warning level) appears in MainView with text "Pricing data unavailable — cost figures may be inaccurate". *Downgraded from [x] to [~] on 2026-08-06 (review finding 34). The original wording — "when `EnsurePricesLoadedAsync()` **throws**" — is the defect: that method cannot throw, because every loader inside `LiteLLMPricingService` catches internally. The `catch` that sets the flag is unreachable, so the banner has never appeared in production and the requirement was never actually met. The trigger has to be `IPricingService.Source == PricingSource.Unknown` (which wave 1 made reachable by publishing a genuinely empty table as `Unknown` instead of falsely claiming `Fallback`), not an exception. That rewiring — `MainViewModel.ApplyPricingSource` — is in flight in the remediation wave running at the time of writing and is not yet committed. Flip this back to [x] once it is committed **and** the banner has been seen with an empty pricing table.*
...
- [x] **DISPATCH-05**: A documented project-wide convention G-1 lands in `CLAUDE.md`: every `IRecipient<T>.Receive(T)` body that mutates `[ObservableProperty]`, calls `INavigationService`, or touches XAML controls MUST wrap the body in `IDispatcherQueue.TryEnqueue`. Exception only with `[ThreadSafeReceive]` attribute or inline justification comment.
- [x] **DISPATCH-06**: A `MessengerThreadingConventionTests` xUnit class enforces G-1 via reflection (or `[RequiresMarshal]` attribute pair if pure reflection proves insufficient — Phase 24 30-min spike decides). Test fails the build when a new `IRecipient<>` handler bypasses the rule.
...
- [x] **L10N-01**: `SettingsViewModel.LastFetchRelativeTime` no longer returns hardcoded EN strings ("just now", "X minutes ago", "X hours ago", "X days ago", "Never"); instead reads from new resw keys `LastFetchJustNow`, `LastFetchMinutesAgo`, `LastFetchHoursAgo`, `LastFetchDaysAgo`, `LastFetchNever` in both DE and EN. *(Key names corrected 2026-08-06: written here as `LastFetchRelative.JustNow` etc., which would not have worked — WinUI3Localizer 2.3.0 splits a key at the first dot and a dotted key silently resolves to an empty string. Phase 27 shipped the single-segment names above; `ResourceCoverageTests` now enforces single-segment keys and asserts every `<data name>` resolves.)*
