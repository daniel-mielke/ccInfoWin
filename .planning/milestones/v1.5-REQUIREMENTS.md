...
**Coverage check (original archive 2026-05-18):** 34/34 REQ-IDs mapped, 0 orphans, 0 duplicates. Each requirement appears in exactly one phase.

---

## Amendment — 2026-05-19 (Post-Archive Hotfix)

Phase 29 was added to the v1.5 milestone on 2026-05-18 (post-archive) to fix the subagent activity-detection gap discovered during visual UAT.

| Requirement | Phase | Status | Description |
|-------------|-------|--------|-------------|
| SUBAGENT-01 | Phase 29 | Complete | `BuildSubagentContext` uses `File.GetLastWriteTimeUtc(file)` as the activity-cutoff source; cutoff applied BEFORE `ReadTailLines` |
| SUBAGENT-02 | Phase 29 | Complete | `SubagentContextData.LastActivity` reflects the mtime, NOT `lastEntry.Timestamp` |
| SUBAGENT-03 | Phase 29 | Complete | `JsonlServiceSubagentTests` xUnit class covers stale-entry-fresh-mtime, all-stale, and LastActivity-tracks-mtime semantics |
| SUBAGENT-04 | Phase 29 | Complete | Visual UAT with 4-parallel-subagent fixture renders 4 of 4 subagents (baseline: 2 of 4 pre-fix) |
| SUBAGENT-05 | Phase 29 | Complete | No regression on existing `JsonlService*` test classes beyond documented baseline |

**Updated total:** 39/39 REQ-IDs mapped. See `v1.5-MILESTONE-AUDIT.md` for the full audit.
