...
- [x] **CTX-04**: User receives autocompact warning at 20K tokens remaining before the *effective*
  max (i.e. before the same `maxTokens - 33K` baseline the progress bar divides by), regardless of
  model. *Amended 2026-08-06 (review finding 23): the original wording said "20K remaining" against
  the raw max, which put the warning threshold 13,000 tokens above the point where the bar already
  displayed 100% — a 200K model at 170,000 tokens showed a saturated red bar and no warning. The
  warning exists to pre-announce autocompact, so it must fire before the saturation point, not
  after it. Implemented in `ModelContextLimits.ShouldWarnAutocompact`; pinned by
  `ShouldWarnAutocompact_WarnsBeforeTheBarSaturates`.*
