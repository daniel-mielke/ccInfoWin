...
/// <summary>
/// Tests for tooltip composition in SessionDisplayItem (D-05, D-06, D-07, D-08, POLISH-04..06).
/// </summary>
public class SessionDisplayTooltipTests
{
    ...
    [Fact]
    public void SessionTimeoutChangedMessage_TriggersRefreshSessionsAsync_TooltipReflectsNewThreshold()
    {
        // We test the message type and its value contract — full integration requires UI thread.
        // Verify: message carries the correct threshold value (the handler reads message.Value).
        const int expectedThreshold = 60;
        var message = new SessionTimeoutChangedMessage(expectedThreshold);

        Assert.Equal(expectedThreshold, message.Value);
    }
